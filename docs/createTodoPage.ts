...
glob("**/*.{hbs,ts,html,css,yml}", {
    ...
    const renderTagToMdTableRow = (entry: Entry): string => `| ${entry.tag} | ${entry.text} | ` +
        ...
        `https://github.com/AnonymerNiklasistanonym/BackupHub/blob/master/${entry.file}#L${entry.line}) |`;
    ...
    fs.mkdirSync(defaultDocsOutputDir, { recursive: true });

    fs.writeFileSync(path.join(defaultDocsOutputDir, "todos.md"),
        "# Open TODOs\n\n" +
